...
$.ajax({
  ...
  data: {
    key: 'G5OBZ-KGNCG-5MAQM-Q4Q7L-JPLUO-LPBBU',
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Kaxiya🍭 %c 你正在访问Kaxiyaの魔法小屋",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Kaxiya🍭") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Kaxiya🍭的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
document.addEventListener('visibilitychange', function () {
  if (document.hidden) {
    //离开当前页面时标签显示内容
    document.title = '不要走啊(⊙﹏⊙)！！';
    clearTimeout(titleTime);
  } else {
    //返回当前页面时标签显示内容
    document.title = '嘿嘿o(*￣▽￣*)ブ';
    //两秒后变回正常标题
...
function createtime() {
  ...
  var start = new Date("01/01/2024 00:00:00"); // 地球逃离太阳系
  ...
  let currentTimeHtml = "";
  currentTimeHtml = `<div style="font-size:13px;font-weight:bold">本站已经运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 地球当前已逃离太阳系 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`;
